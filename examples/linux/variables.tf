...
variable "template_name" {
  description = "Name of the Linux VM template to clone from."
  type        = string
}
...
variable "guest_id" {
  description = "VMware guest OS identifier (e.g. rhel9_64Guest, ubuntu64Guest)."
  type        = string
}
...
variable "time_zone" {
  description = "Olson time zone string (e.g. America/New_York, Europe/London)."
  type        = string
  default     = null
}
...
variable "enable_disk_uuid" {
  description = "Expose disk UUIDs to the guest OS. Required by some workloads (e.g. Kubernetes, vSAN)."
  type        = bool
  default     = true
}

variable "nested_hv_enabled" {
  description = "Expose VMX flag to the guest to enable nested hardware virtualization."
  type        = bool
  default     = false
}
